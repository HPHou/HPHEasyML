...
def atomic_benchmark_estimator(estimator, X_test, n_bulk_repeats, verbose=False):
    ...
    if verbose:
        print("atmoic_benchmark runtime:",min(runtimes),
...
def plot_benchmark_throughput(throughputs, configuration):
    ...
    cls_infos = ["%s \n(%d%s)" % (estimator_conf['name'],
                ...
                estimator_conf['complexity_label'])
                for estimator_conf in configuration['estimators']]
    cls_values = [throughputs[estimator_conf['name']]    
                for estimator_conf in configuration['estimators']]
    ...
    ax.set_ylim((0, ymax))
    ax.set_ylabel('Throughput (predicitons/sec)')
...
configuration = {
    ...
    'estimators': [
        {'name': 'Linear Model',
        'instance':SGDRegressor(penalty='elasticnet', alpha=0.1, l1_ratio=0.25,fit_intercept=True,tol=1e-4),
        'complexity_label': 'non-zero coefficients',
        'complexity_computer': lambda clf: np.count_nonzero(clf.coef_)},
        {'name': 'Linear Model',
        'instance':SGDRegressor(penalty='elasticnet', alpha=0.1, l1_ratio=0.9,fit_intercept=True,tol=1e-4),
        'complexity_label': 'non-zero coefficients',
        'complexity_computer': lambda clf: np.count_nonzero(clf.coef_)},
        {'name':'RandomForest',
